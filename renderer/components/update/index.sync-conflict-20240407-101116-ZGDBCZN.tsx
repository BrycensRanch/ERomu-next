...
function Update() {
  ...
  useEffect(() => {
    ...
    window.ipcRenderer.on('check-update', checkUpdate);
    return () => {
  ...
  if (updateError) {
    window.ipcRenderer.send('update-error', updateError);
  }
  return (
    <>
      <Modal
      ...
      <Button disabled={checking} onClick={checkUpdate} className="mx-auto flex flex-col text-4xl" variant="solid">
